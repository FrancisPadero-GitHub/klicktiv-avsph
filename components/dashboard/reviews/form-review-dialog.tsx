...
export function AddEditReviewDialog({
  ...
  const job_id = useWatch({ control, name: "job_id" });
  const review_type_id = useWatch({ control, name: "review_type_id" });
  const payment_method_id = useWatch({ control, name: "payment_method_id" });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-h-[90vh] overflow-y-auto sm:max-w-3xl break-all"
